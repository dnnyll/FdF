...
void	draw_grid(t_map *map)
{
	...
	while (i < map->x)
	{
		...
		while (j < map->y)
		{
			...
			{
				printf("Skipping out-of-bounds access at i: %d, j: %d\n", i, j);
			}
			printf("map->conversion_grid[%d][%d]: %f\n", i, j, map->conversion_grid[i][j]);
			...
			mlx_put_image_to_window(map->mlx_ptr, map->window->win_ptr, map->render->img_ptr, map->iso->offset_x, map->iso->offset_y);
