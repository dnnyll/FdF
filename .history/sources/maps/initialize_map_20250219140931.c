...
/*   Updated: 2025/02/19 14:09:31 by daniefe2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	map->window->win_width = 3200;
	map->window->win_height = 2048;
	...
	map->render->size_line = 0;
	map->render->bpp = 0;
	map->render->buffer = NULL;
	map->render->endian = 0;
	map->render->
	map->render->
}
...
t_map	*allocate_map(void)
{
	...
	map->render = (t_render *)malloc(sizeof(t_render));
	if (!map->iso || !map->colour || !map->window || !map->render)
	{
		free(map->iso);
		free(map->colour);
		free(map->window);
		free(map->render);
		free(map);
		ft_printf("Error allocating structure memory\nAll stuctures were freed");
		return (NULL);
	}
	return (map);
}
