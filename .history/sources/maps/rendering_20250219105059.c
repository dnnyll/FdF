...
/*   Updated: 2025/02/19 10:50:59 by daniefe2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	create_img (t_map *map)
{
	...
	while (i < map->height)
	{
		...
		while (j < map->width)
		{
			put_pixel(map->render->buffer, map->colurs_matri[i][j], map->window->win_width, map->window->win_height, map->render->bpp, map->render->size_line);
			j++;
		}
		i++;
	}
