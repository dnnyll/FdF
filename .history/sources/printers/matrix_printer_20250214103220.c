...
/*   Updated: 2025/02/14 10:32:20 by daniefe2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void print_c_z_matrix(t_map *map)
{
    ...
    while (i < map->y)
    {
        ...
        while (j < map->x)
        {
            ...
            while (map->c_z_matrix[i][j][k] != NULL) // Assuming the third dimension is NULL-terminated
            {
                if (map->c_z_matrix[i][j][k])
                    ft_printf("%s", map->c_z_matrix[i][j][k]);
                else
                    ft_printf("NULL");
                
                if (map->c_z_matrix[i][j][k + 1] != NULL)
                    ft_printf("\t"); // Space between values at the third dimension level
                k++;
            }
            if (j < map->x - 1)
                ft_printf("\t"); // Space between columns
            j++;
        }
        ft_printf("\n"); // End of a row
        i++;
