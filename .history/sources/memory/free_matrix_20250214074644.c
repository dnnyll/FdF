...
/*   Updated: 2025/02/14 07:46:44 by daniefe2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	free_coordinates_grid(t_map *map)
{
	...
	if (map->coordinates_grid)
	{
		while (i < map->y)
		{
			if (map->coordinates_grid[i])
			{
				j = 0;
				while (j < map->x)
				{
					if (map->map->coordinates_grid[i][j])
						free(map->map->coordinates_grid[i][j]);
					j++;
				}
				free(map->map->coordinates_grid[i]);
			}
			i++;
		}
		free(map->map->coordinates_grid);
	}
}
