...
/*   Updated: 2025/02/14 10:14:00 by daniefe2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	free_c_z_matrix(t_map *map)
{
	...
	if (map->c_z_matrix)
	{
		...
		while (i < map->y)
		{
			if (map->c_z_matrix[i])  // Ensure row exists
			{
				j = 0;
				// Loop through columns
				while (j < map->x)
				{
					if (map->c_z_matrix[i][j])  // Ensure column exists
					{
						free(map->c_z_matrix[i][j]);  // Free individual element
					}
					j++;
				}
				free(map->c_z_matrix[i]);  // Free the row
			}
			i++;
		}
		// Free the array of row pointers (map->c_z_matrix)
		free(map->c_z_matrix);
