...
/*   Updated: 2025/02/18 10:06:08 by daniefe2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!map->char_matrix_stash)
		return; // Nothing to free if already NULL

	int row = 0;
	while (row < map->y)
	{
		if (map->char_matrix_stash[row]) // Free each row if allocated
		{
	...
	free(map->char_matrix_stash); // Finally, free the array of pointers
