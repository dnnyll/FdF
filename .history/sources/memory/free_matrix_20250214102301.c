...
/*   Updated: 2025/02/14 10:23:01 by daniefe2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		while (row < map->y)
		{
			if (map->c_z_matrix[row])
			{
				...
				while (col < map->x)
				{
					...
					{
						k = 0;
						while (map->c_z_matrix[row][col][k] != '\0')  // Iterating through the third dimension.
						{
							free(map->c_z_matrix[row][col][k]);  // Free each element in the third dimension.
							k++;
						}
						free(map->c_z_matrix[row][col]);  // Free the second dimension.
					}
					col++;
				}
				free(map->c_z_matrix[row]);  // Free the first dimension.
			}
			row++;
		}
		free(map->c_z_matrix);  // Finally, free the top-level pointer.
	}
}
