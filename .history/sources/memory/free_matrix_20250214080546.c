...
/*   Updated: 2025/02/14 08:05:46 by daniefe2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	free_c_z_matrix(t_map *map)
{
	...
	while (i < map->y)
	{
		if (map->c_z_matrix[i])
		{
			...
			while (j < map->x)
			{
				free(map->c_z_matrix[i][j]); // Free individual elements
				j++;
			}
			free(map->c_z_matrix[i]); // Free row array
		}
		i++;
	}
	free(map->c_z_matrix); // Free main array
}
